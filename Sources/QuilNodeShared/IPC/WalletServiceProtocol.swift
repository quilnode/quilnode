...
/// Codable metadata shared by the unprivileged controller and authenticated
/// wallet service. Private key bytes are deliberately absent from every type.
...
/// User-owned intent passed across the privilege boundary. The daemon treats
/// every field as hostile and never accepts private key bytes over IPC.
